...
function queue_print () { // {{{
	selected_printer.call ('axis_get_current_pos', [0], {}, function (x) {
		selected_printer.call ('axis_get_current_pos', [1], {}, function (y) {
			var r = selected_printer.reference;
			var sina = Math.sin (selected_printer.angle);
			var cosa = Math.cos (selected_printer.angle);
			selected_printer.call ('queue_print', [get_queue (), [x[1] - (r[0] * cosa + r[1] * sina), y[1] - (r[1] * cosa - r[0] * sina)], selected_printer.angle], {});
		});
	});
}
// }}}
...
function update_variables () { // {{{
	...
	for (var i = 0; i < visibles[port].titles.length; ++i) {
		function check_titles (items, objs, add, remove) {
...
function move_axis (axes, amounts, update_lock, pos) { // {{{
	...
	if (pos.length < axes.length) {
		selected_printer.call ('axis_get_current_pos', [axes[pos.length]], {}, function (result) {
...
function set_reference (x, y, ctrl) { // {{{
	selected_printer.call ('axis_get_current_pos', [0], {}, function (current_x) {
		selected_printer.call ('axis_get_current_pos', [1], {}, function (current_y) {
			var dx = Math.cos (selected_printer.angle) * (x - selected_printer.reference[0]) + Math.sin (selected_printer.angle) * (y - selected_printer.reference[1]);
			var dy = Math.cos (selected_printer.angle) * (y - selected_printer.reference[1]) - Math.sin (selected_printer.angle) * (x - selected_printer.reference[0]);
			...
			if (ctrl)
				...
				selected_printer.call ('goto', [[current_x[1] + dx, current_y[1] + dy]], {'cb': true}, done);
		});
	});
}
// }}}
...
function update_canvas_and_spans (update_lock) { // {{{
	...
	printer.call ('axis_get_current_pos', [0], {}, function (x) {
		printer.call ('axis_get_current_pos', [1], {}, function (y) {
			printer.call ('axis_get_current_pos', [2], {}, function (z) {
				if (update_lock)
...
function redraw_canvas (x, y) { // {{{
	...
	case 0:
		...
		printerwidth = 2 * Math.max (xaxis.limit_max_pos, -xaxis.limit_min_pos) + 10;
		printerheight = 2 * Math.max (yaxis.limit_max_pos, -yaxis.limit_min_pos) + 10;
		outline = function (c) {
			c.beginPath ();
			// Why does this not work?
			//c.rect (xaxis.limit_min_pos, yaxis.limit_min_pos, xaxis.limit_max_pos - xaxis.limit_min_pos, yaxis.limit_max_pos - y.limit_min_pos);
			c.moveTo (xaxis.limit_min_pos, yaxis.limit_min_pos);
			c.lineTo (xaxis.limit_min_pos, yaxis.limit_max_pos);
			c.lineTo (xaxis.limit_max_pos, yaxis.limit_max_pos);
			c.lineTo (xaxis.limit_max_pos, yaxis.limit_min_pos);
			c.lineTo (xaxis.limit_min_pos, yaxis.limit_min_pos);
			c.stroke ();
		}
		break;
	case 1:
		...
		var dy = [1, -.5, -.5];
		var intersects = [];
		function intersect (x0, y0, r, x1, y1, dx1, dy1, positive) {
	...
	if (l !== null && l[1] !== r) {
		...
		selected_printer.angle = Math.atan2 (target[1], target[0]) - Math.atan2 (real[1], real[0]);
	}
	...
	// Rotate view according to angle.
	c.rotate (selected_printer.angle);
	...
	if (l === null) {
		...
		c.arc (true_pos[0], true_pos[1], 3, 0, 6.2832);
		c.fill ();
	}
	else {
...
function key_move (key, shift, ctrl) { // {{{
	...
	if (key == 13) {
		if (selected_printer.lock !== null) {
			...
			// Move an arbitrary axis 0 mm, to update the canvas.
			move_axis ([0], [0]);
		}
		else {
			// Move an arbitrary axis 0 mm, to update the canvas; also update the lock.
			move_axis ([0], [0], true);
		}
	}
	// 27: escape
	...
	// 37: left
		...
		move_axis ([0, 1], [-value * Math.cos (-selected_printer.angle), -value * Math.sin (-selected_printer.angle)]);
	// 38: up
	else if (key == 38)
		move_axis ([0, 1], [value * -Math.sin (-selected_printer.angle), value * Math.cos (-selected_printer.angle)]);
	// 39: right
	else if (key == 39)
		move_axis ([0, 1], [value * Math.cos (-selected_printer.angle), value * Math.sin (-selected_printer.angle)]);
	// 40: down
